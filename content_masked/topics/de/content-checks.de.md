{
  ...
  "url": "[URL_BASE_TOPICS]synthetic-monitoring/transaktionen/inhaltspruefung",
...
Du kannst Inhaltsprüfungen während des [Aufzeichnungsprozesses]([LINK_URL_1]) oder über den Schritte-Editor des Prüfobjekts einfügen.

Füge eine Inhaltsprüfung hinzu:

1. Öffne das Transaktionsprüfobjekt, das du ändern möchtest.
2. Wechsele zur Registerkarte [SHORTCODE_1] Schritte [SHORTCODE_2].
3. Scrolle zu dem Schritt, dem du eine Inhaltsprüfungsaktion hinzufügen möchtest.
4. Klicke auf [SHORTCODE_3] Aktion hinzufügen [SHORTCODE_4]. Du siehst in der Liste, dass der Aktionstyp zur Inhaltsprüfung über eine grüne „Test“-Bezeichnung verfügt.
![Screenshot Inhaltsprüfung bei Transaktion hinzufügen]([LINK_URL_2])
5. Wähle den entsprechenden Aktionstyp aus dem Pop-up-Fenster. Nach deiner Wahl erscheint die neue Aktion im Editor.
   ![Screenshot Inhaltsprüfung in Transaktionsschritt]([LINK_URL_3])
  Dieser Screenshot zeigt dir den Aktionstyp *Prüfe Elementinhalt*.
6. Gib die Einstellungen für deine Inhaltsprüfung ein. Die Einstellungsoption variieren je nach ausgewähltem Aktionstyp. Der Abschnitt [Typen von Inhaltsprüfungen]([LINK_URL_4]) bietet weitere Informationen zu den Optionen eines Typs.
7. Klicke auf die [SHORTCODE_5]Speichern[SHORTCODE_6]-Schaltfläche, um alle Änderungen zu sichern.

Die obigen Anweisungen sind für den visuellen Editor. Du kannst Schritte im [Skripteditor]([LINK_URL_5]) hinzufügen und ändern.

## Typen von Inhaltsprüfungen [ANCHOR_1]

Derzeit besteht die Möglichkeit, zwischen unterschiedlichen Inhaltsprüfungen auszuwählen.

### Prüfe Elementinhalt [ANCHOR_2]
...
![Screenshot Testaktion Prüfe Elementinhalt oben]([LINK_URL_6])

Du siehst ein Drop-down-Menü, um zwischen den beiden Selektorentypen zu wechseln. Weitere Infos zu Selektoren findest du in den Knowledge-Base-Artikeln [Selektoren nutzen]([LINK_URL_7]) und [Selektoralternativen]([LINK_URL_8]).
...
- beinhaltet
- enthält nicht (siehe auch [Negative Inhaltsprüfungen]([LINK_URL_9]))
- stimmt mit Regular Expression überein
- stimmt nicht mit Regular Expression überein (siehe auch [Negative Inhaltsprüfungen]([LINK_URL_10]))
...
**Warte bis** – gib eine Wartebedingung für diesen Test des Prüfobjekts ein. Erfahre mehr im Artikel [Warte auf Element]([LINK_URL_11]). Hinweis: Das Feld „Warte bis“ ist nicht für den Aktionstyp *Prüfe Dokumentinhalt* verfügbar.
...
**Variable zuweisen** – gib einen Variablennamen im Format [INLINE_CODE_1] ein. Lies unseren Artikel zur [Nutzung von Transaktionsvariablen]([LINK_URL_12]), um weitere Infos zu erhalten.

**Shadow DOM Host** – gib einen [Shadow DOM]([LINK_URL_13]) Host und sogar eingebettete Shadow DOMs an.

Wenn du direkt mit dem JSON-Transaktionsskript im [Skripteditor]([LINK_URL_14]) arbeitest, muss der Prüfungstyp *Prüfe Elementinhalt* folgendes Format einhalten:

[CODE_BLOCK_1]

Wenn Parameter wie [INLINE_CODE_2], [INLINE_CODE_3] und [INLINE_CODE_4] optional sind, kannst du sie auslassen. Die verfügbaren [INLINE_CODE_5]-Parameterwerte sind [INLINE_CODE_6], [INLINE_CODE_7], [INLINE_CODE_8] und [INLINE_CODE_9]. Dieses Beispiel verifiziert, dass das (Beispiel-)Element mit dem XPath-Selektor [INLINE_CODE_10] den String „Example text“ beinhaltet. Natürlich kannst du auch den CSS-Selektor nutzen: Nutze den Wert [INLINE_CODE_11], statt [INLINE_CODE_12].
...
[CODE_BLOCK_2]
Beachte, dass die Parameter [INLINE_CODE_13], [INLINE_CODE_14] und [INLINE_CODE_15] optional sind, sodass du sie auslassen kannst.
...
[CODE_BLOCK_3]
Beachte, dass die Parameter [INLINE_CODE_16], [INLINE_CODE_17] und [INLINE_CODE_18] optional sind, sodass du sie auslassen kannst.

## Negative Inhaltsprüfungen [ANCHOR_3]
...
Verwende einen [regulären Ausdruck]([LINK_URL_15]), wo dies sinnvoll ist. Wenn deine Seite zum Beispiel nach einer Suche „15 Ergebnis(se) gefunden“ anzeigt, richte eine Inhaltsprüfung für [INLINE_CODE_19] ein und setze die Bedingung auf **stimmt mit Regular Expression überein**.
...
Ein guter Selektor ist auf der Seite einzigartig. Das Einsetzen eines einzigartigen Werts sorgt dafür, dass Uptrends nach dem richtigen Element sucht. Wenn möglich, nutze das ID-Attribut des Elements, da dies sich im Allgemeinen einzig auf das gewählte Element bezieht. Zum Beispiel [INLINE_CODE_20] oder [INLINE_CODE_21].

Mehr zu Selektoren findest du in den Knowledge-Base-Artikeln [Selektoren nutzen]([LINK_URL_16]) und [Selektoralternativen]([LINK_URL_17]).

## Vorbehalte, Fallstricke und Tipps

**Achte auf Groß- und Kleinschreibung**. Beim Einsatz von CSS erscheint Text häufig in Großbuchstaben auf der Seite, während im DOM die übliche Groß- und Kleinschreibung verwendet wird. Das heißt, der Selektor könnte [INLINE_CODE_22] lauten, aber der Text auf der Seite könnte als „HELLO“ erscheinen. Der Selektor sollte die Information des DOM wiedergeben und der von dir eingegebene String sollte widerspiegeln, was tatsächlich auf der Seite steht.
